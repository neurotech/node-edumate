...
edumate.open = function (config) {
  return new Promise(function (resolve, reject) {
    db2.open('DRIVER={DB2};DATABASE=' + config.suffix + ';HOSTNAME=' + config.host + ';UID=' + config.username + ';PWD=' + config.password + ';PORT=' + config.port + ';PROTOCOL=TCPIP;CURRENTSCHEMA=EDUMATE', function (err, conn) {
      if (err) {
